...
SQLCohortsCreate<-function(path, timeoutInSeconds, cohortDatabaseSchema, cohortTable, connectionDetails){
  #path<-pathtoSQL
  ...
  completesql<-sprintf("SELECT cohort_definition_ID, COUNT(*) subjectCt
                          FROM %s
                          GROUP BY cohort_definition_ID
                          ORDER BY cohort_definition_ID", paste(cohortDatabaseSchema, '.',cohortTable, sep=""))
